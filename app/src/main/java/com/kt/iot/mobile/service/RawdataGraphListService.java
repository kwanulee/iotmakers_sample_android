                ...
                if (device.getTagStrmList() != null && device.getTagStrmList().size() > 0) {
                    ...
                    //while(isRunning) {
                        ...
                        try {
                            ...
                            TagStrmApiResponse response = tagStrmApi.getTagStrmLog(device.getSpotDevId(), "100000", "10");

                            if (response.getResponseCode().equals(ApiConstants.CODE_OK)) {

                                android.util.Log.i("TAG_STream","response.getResponseCode() = Response OK");

                                for (TagStrm tag : device.getTagStrmList()) {

                                    if (tag.getTagStrmPrpsTypeCd().equals(TagStrmApi.TAGSTRM_DATA)) {

                                        android.util.Log.i("TAG_STream","TagStrm tag = "+tag.getTagStrmId());
